...
function emptyValuesForTemplate(template: PostTemplate): Record<string, string> {
    const initial: Record<string, string> = {};
    template.placeholders.forEach((p) => { initial[p] = ''; });
    return initial;
}

function TemplateCustomizeDrawer({ template, onClose }: { template: PostTemplate; onClose: () => void }) {
    const navigate = useNavigate();
    const [values, setValues] = useState(() => emptyValuesForTemplate(template));
    ...
    const isValid = useMemo(
        () => template.placeholders.every((p) => values[p]?.trim().length > 0),
        [template, values],
    );
...
export function TemplateCustomizeModal({ template, onClose }: Props) {
    return (
        <AnimatePresence>
            {template && (
                <TemplateCustomizeDrawer key={template.id} template={template} onClose={onClose} />
            )}
        </AnimatePresence>
    );
}
